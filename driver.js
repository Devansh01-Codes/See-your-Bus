  ...
  let lastStop=null;           /* { lat, lng, name, addr } */
  ...
  function openSearch(mode){
    ...
    if(!currentLat) navigator.geolocation.getCurrentPosition(p=>{ currentLat=p.coords.latitude; currentLng=p.coords.longitude; },()=>{});
  }

  /* ════════ STEP 2: Search modal ════════ */

  /* Build descriptive names from Nominatim address object */
  ...
  document.getElementById('lastStopInput').addEventListener('input',e=>{
    ...
    clearStop();   /* always clear previous selection when user types */
    clearTimeout(sTimer);
  ...
  async function fetchSugg(q){
    ...
    try{
      /* zoom=18 = street/building level; addressdetails=1 = full address breakdown */
      ...
      if(currentLat&&currentLng){
        /* Pass 1: tight 30 km box → very local */
        ...
        /* Pass 2: widen to ~150 km if fewer than 4 local hits */
        if(data.length<4){
  ...
  async function startTracking(){
    ...
    watchId=navigator.geolocation.watchPosition(async pos=>{
      ...
      if(now-lastUpdate>3000){
        lastUpdate=now;
        try{ await set(ref(db,`active_buses/${driverUID}`),{latitude,longitude,heading:heading??null,speed:speed??null,accuracy:accuracy??null,timestamp:now,isLive:true,tripMode,driverName,lastStop:lastStop?{lat:lastStop.lat,lng:lastStop.lng,name:lastStop.name}:null}); }catch{}
      }
    },err=>toast('Location error: '+err.message,true),{enableHighAccuracy:true,timeout:10000,maximumAge:0});

    isLive=true;
    stopsListener=ref(db,`bus_students/${driverUID}`);
    onValue(stopsListener,snap=>{
      ...
      Object.entries(data).forEach(([uid,stop])=>{
        ...
        addStudentMarker(stop); showStopNotif(stop);
  ...
  async function drawRoute(fLat,fLng,tLat,tLng,wps=[]){
    ...
    try{
      const d=await(await fetch(`https://router.project-osrm.org/route/v1/driving/${fLng},${fLat}${wc?';'+wc:''};${tLng},${tLat}?overview=full&geometries=geojson`)).json();
      if(d.code!=='Ok') return;
      if(routeLayer) mainMap.removeLayer(routeLayer);
      routeLayer=L.geoJSON(d.routes[0].geometry,{style:{color:'#4361ee',weight:5,opacity:.85,lineCap:'round',lineJoin:'round'}}).addTo(mainMap);
  ...
  function addStudentMarker(stop){
    if(!mainMap) return;
    L.marker([stop.lat,stop.lng],{icon:L.divIcon({className:'',html:`<div style="width:14px;height:14px;border-radius:50%;background:var(--primary);border:3px solid #fff;box-shadow:0 0 0 3px rgba(67,97,238,.25);"></div>`,iconSize:[14,14],iconAnchor:[7,7]})}).addTo(mainMap).bindPopup(`<b>Student Pickup</b><br>${stop.name||'Nearby stop'}`);
  }
  ...
  async function stopTracking(){
    ...
    try{ await set(ref(db,`active_buses/${driverUID}`),{latitude:null,longitude:null,isLive:false,tripMode:null,driverName:null,lastStop:null}); await set(ref(db,`bus_students/${driverUID}`),null); }catch{}
  ...
  function busIcon(){
    return L.divIcon({className:'',html:`<div style="background:#1a73e8;border-radius:50% 50% 50% 0;transform:rotate(-45deg);width:40px;height:40px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.3);border:2px solid #fff;"><i class="fa-solid fa-bus" style="color:#fff;font-size:16px;transform:rotate(45deg);"></i></div>`,iconSize:[40,40],iconAnchor:[20,40]});
  }
  ...
  navigator.geolocation.getCurrentPosition(p=>{ currentLat=p.coords.latitude; currentLng=p.coords.longitude; },()=>{},{enableHighAccuracy:true});
