...
/* ✅ FIX: Helper to show the correct screen and hide the loading spinner */
function showScreen(screen) {
  const loading    = document.getElementById('authLoading');
  const loginScr   = document.getElementById('loginScreen');
  const adminShell = document.getElementById('adminShell');
  if (loading)    loading.style.display    = 'none';
  if (loginScr)   loginScr.style.display   = screen === 'login' ? 'flex' : 'none';
  if (adminShell) adminShell.style.display = screen === 'admin' ? 'block' : 'none';
}
...
auth.onAuthStateChanged((user) => {
  ...
  // User logged in — verify they exist in /admins in the database
  db.ref('admins/' + user.uid).once('value').then(snap => {
    if (!snap.exists()) {
      // Not an admin — sign out and show error on login screen
      showScreen('login');
      document.getElementById('loginErr').textContent = 'Access Denied! Not an admin account.';
      auth.signOut();
      return;
    }

    // ✅ Confirmed admin — show the shell
    showScreen('admin');
    ...
    // Only init listeners once — prevents duplicate Firebase listeners on re-auth
    ...
    showScreen('login');
    document.getElementById('loginErr').textContent = 'Auth error: ' + err.message;
    auth.signOut();
  });
});
...
// onAuthStateChanged handles showing the shell after login
}
